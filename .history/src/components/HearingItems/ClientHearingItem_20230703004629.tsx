...
export const ClientCreationHearingItem = (handleFieldChange: any) => {
  const HearingItem = [
    ...
    { label: "既存サイト：有無", type: "text", value: "" },
  ...
  return (
    <React.Fragment>
      {HearingItem.map((field, index) => (
        <div key={index} className="grid grid-cols-2 items-center w-[700px]">
          <p className="w-fit whitespace-nowrap grid-flow-row">{field.label}</p>
          {field.type === "text" ? (
            <input
              type="text"
