...
export const ClientCreationHearingItem = ({ fields, handleFieldChange }: any) => {
  const isFirstNewSiteLabel = (index: number) => {
    if (index >= 0 && index < fields.length) {
      const currentLabel = fields[index].label;
      if (currentLabel.startsWith("新規サイト：")) {
        for (let i = index - 1; i >= 0; i--) {
          const previousLabel = fields[i].label;
          if (previousLabel.startsWith("新規サイト：")) {
            return false;
          } else if (!previousLabel.startsWith("既存サイト：")) {
            return true;
          }
        }
        return true;
    ...
    <React.Fragment>
      {fields.map((field: any, index: number) => (
        <div key={index} className="grid grid-cols-3 w-[700px]">
          <p
            className={`w-fit whitespace-nowrap col-span-1 p-2 ${
              isFirstNewSiteLabel(index) ? "border-t-2 border-gray-300" : ""
            }`}
          >
            {field.label}
          </p>
