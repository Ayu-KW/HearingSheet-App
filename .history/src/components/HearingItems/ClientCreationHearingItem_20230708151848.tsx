...
export const initialFields = [
  ...
  { label: "新規サイト：案件カテゴリ", type: "select", value: "", options: ["新サイト作成", "リニューアル", "既存サイト修正", "その他"] },
    ...
    if (index >= 0 && index < fields.length) {
      ...
      if (currentLabel.startsWith("既存サイト：")) {
        for (let i = index - 1; i >= 0; i--) {
          ...
          if (previousLabel.startsWith("既存サイト：") || previousLabel.startsWith("新規サイト：")) {
            return false;
          }
        }
        return true;
        ...
        for (let i = index - 1; i >= 0; i--) {
          const previousLabel = fields[i].label;
          if (previousLabel.startsWith("新規サイト：") || previousLabel.startsWith("既存サイト：")) {
            return false;
          }
        }
        return true;
    ...
    <React.Fragment>
      {fields.map((field: any, index: number) => (
        <div key={index} className="grid grid-cols-3 w-[700px]">
          <div className={`col-span-3 ${isFirstLabelGroup(index) ? 'border-t-2 border-gray-300' : ''}`}>
