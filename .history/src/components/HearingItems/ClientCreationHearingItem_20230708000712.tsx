...
export const ClientCreationHearingItem = ({ fields, handleFieldChange }: any) => {
  const existingSiteFields = fields.filter((field: any) =>
    field.label.startsWith("既存サイト")
  );
  const newSiteFields = fields.filter((field: any) =>
  ...
  return (
    <React.Fragment>
      <div className="mb-6">
        <h3 className="text-lg font-medium mb-2">既存サイト</h3>
        {existingSiteFields.map((field: any, index: number) => (
          <div key={index} className="grid grid-cols-3 items-center w-[700px]">
            <p className="w-fit whitespace-nowrap col-span-1">{field.label}</p>
            {/* フィールドのレンダリング */}
          </div>
      ...
      <div className="mb-6">
        <h3 className="text-lg font-medium mb-2">新規サイト</h3>
        {newSiteFields.map((field: any, index: number) => (
          <div key={index} className="grid grid-cols-3 items-center w-[700px]">
            <p className="w-fit whitespace-nowrap col-span-1">{field.label}</p>
            {/* フィールドのレンダリング */}
          </div>
