...
const CustomerServiceName = ({ allClientData }: any) => {
  return (
    <main className="lg:w-2/3 md:w-4/5 sm:w-11/12 m-auto font-mono py-36">
      <div className="text-center mb-10">
        ...
        <p className="text-2xl mt-2">
          {allClientData.ClientComponyName} /{" "}
          {allClientData.ClientServiceName || "（未入力）"}
        </p>
      </div>
      <dl className="grid grid-cols-4 border border-cyan-600 mt-5">
        <ClientHearingItem clientPage={allClientData} />
      </dl>
    </main>
  );
};
