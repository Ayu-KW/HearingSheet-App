...
const CustomerServiceName = ({ clientPage }: any) => {
  ...
  return (
    <main className="w-11/12 md:max-w-4xl m-auto font-mono py-36 px-4">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-bold text-sky-700">顧客詳細ページ</h2>
        <p className="text-2xl mt-2">
          {clientPage.ClientComponyName} /<span className="block md:hidden"></span>{" "}
          {clientPage.ClientServiceName || "（未入力）"}
        </p>
      </div>
      <dl className="grid grid-cols-1 sm:grid-cols-4 border border-cyan-600 mt-5">
        <ClientHearingItem clientPage={clientPage} />
      </dl>
      <button className="py-2 px-4 mt-4 rounded border border-cyan-800 border-separate">
        編集する（未実装）
      </button>
    </main>
  );
